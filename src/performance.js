...
const PolarData = require("./polars.json");
...
module.exports = function() {
  ...
  function loadPolar(polarName) {
    if ( polarName !== polarPerf.polarName || polarPerf.polarName === undefined) {
      resolvePolar(polarName, (uri, polarSpec) => {
        if ( polarSpec !== undefined ) {
          ...
          console.error("Error processing polar ",polarName, e);
        }
      });
    }
  }
  function finishLoadPolar(polarName, polar) {
    ...
    polarPerf.polarName = polarName;
    console.log("Resolved and loaded ", polarName);
  }

  function resolvePolar(polarName, cb) {
    cb(polarName, PolarData.polars[polarName]);
  }
  /**
  ...
  function buildFinePolarTable(polar) {
    ...
    var t = Date.now();
    console.log("Starting fine polar");
    ...
    console.log("Finished fine polar build, in SI units ",(Date.now()-t),"ms");
    return finePolar;
  }
  ...
  var polarPerf = {
    ...
    availablePolars: Object.keys(PolarData.polars), 
    init: function(polarName) {
      loadPolar(polarName);
    },
    performanceForSpeed: function(tws) {
      return calcSpeedCurve(polarPerf.polar, tws);
    },
    calc: function (tws, twa) {
      var targets = calcTargetAngleSpeed(polarPerf.polar, tws, twa);
      var performance = getPerformance(polarPerf.polar, tws, twa, 0);
      if ( targets.upwind ) {
        if ( Math.abs(twa) > Math.abs(targets.twa)) {
          targets.twa = twa;
          targets.stw = performance.polarSpeed;
        }
      } else {
        if ( Math.abs(twa) < Math.abs(targets.twa)) {
          targets.twa = twa;
      ...
      return {
        targets: {
          ...
          twa: targets.twa,
          tws: tws,
          upwind: targets.upwind
        },
        stw: performance.polarSpeed,
        twa: twa,
        tws: tws
      }
    },
